...
const DoctorAppointments = () => {
    ...
    const handleAction = async (id, action) => {
        const actionMessages = {
            confirm: { success: 'Cita confirmada exitosamente', error: 'Error al confirmar la cita' },
            cancel: { success: 'Cita cancelada exitosamente', error: 'Error al cancelar la cita' },
            complete: { success: 'Cita finalizada exitosamente', error: 'Error al finalizar la cita' }
        };

        try {
            setLoadingAction(`${id}-${action}`);
            await apiClient.put(`/appointments/${id}/${action}`);
            alert(actionMessages[action].success);
            fetchAppointments();
        } catch (error) {
            console.error(`Error al ${action} cita:`, error);
            const errorMsg = error.response?.data?.message || actionMessages[action].error;
            alert(errorMsg);
        } finally {
                                        ...
                                        <td className="p-4 flex justify-center gap-2">
                                            {appt.status === 'pending' && (
                                                <>
                                                    <ActionButton
                                                        ...
                                                        title="Confirmar"
                                                        disabled={loadingAction === `${appt.id}-confirm`}
                                                    />
                                                    <ActionButton
                                                        ...
                                                        title="Cancelar"
                                                        disabled={loadingAction === `${appt.id}-cancel`}
                                                    />
                                                </>
                                            )}
                                            {appt.status === 'confirmed' && (
                                                <button
                                                    ...
                                                    disabled={loadingAction === `${appt.id}-complete`}
                                                    className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
                                                    <CheckCircle size={14} />
                                                    {loadingAction === `${appt.id}-complete` ? 'Finalizando...' : 'Finalizar'}
                                                </button>
                                            )}
...
const ActionButton = ({ onClick, icon: Icon, color, title, disabled }) => (
    <button
        onClick={onClick}
        title={title}
        disabled={disabled}
        className={`p-1.5 rounded-lg transition-colors ${color} disabled:opacity-50 disabled:cursor-not-allowed`}
    >
        <Icon size={16} />
    </button>
);
